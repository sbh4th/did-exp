...
write_rds(d, here("data", "did-sim-data.rds"))
```
...
In contrast to the `did` package (or `csdid` in Stata) from CS that automatically provides different aggregated ATTs, with ETWFE we need to do this by hand.^[It's worth noting that there is an R [package](https://grantmcdermott.com/etwfe/index.html) called `etwfe` by Grant McDermott that is wonderful and was helpful in coding this up. However, I think it is often easier to do it manually to help understand what the package is doing.] The `marginaleffects` package makes this pretty straightforward. The basic idea here is to use the extended TWFE model to generate marginal predictions (and contrasts of marginal predictions) over different combinations of covariates (e.g., simple averaging, groups, time-since-treatment). 
...
The dynamic estimates above only apply to the post-intervention period, in contrast to the non-parametric approach by CS above, which also gives estimates of ATTs prior to any intervention (i.e., assessing whether the pre-trends may be suggest a violation of the parallel trends assumption. In the ETWFE approach of @{Wooldridge:2021aa} he makes a case that, in fact, parallel trends requires the assumption that any 'pre-trends' are zero, so that a fully saturated  'event-study' type model that allows for heterogeneity in the pre-trends means that we cannot, strictly speaking, interpret the group-time estimates in the post period as ATTs, which is why he typically does not include them (and why the `etwfe` package also does not produce estimates of pre-treatment effects because they are set to zero for estimation). 

However, we may still want to conduct some pre-intervention testing, and @{Wooldridge:2021aa} suggests restricting to the untreated observations (i.e., subsetting to `treat=0`) and conducting the same kind of fully interactive model with the treatment cohorts and time. Below we show how this can be implemented, as well as a way to plot them together with the dynamic ATT effects we saw above. We will create an 'ever treated' variable to interact with the cohort and time interactions to make getting the marginal effects easier. We can also use the `marginaleffects` package to calculate a joint test that all of the product terms between cohorts and time are equal to zero in the pre-period.

```{r pre}
d <- d %>% 
  mutate(evtreat = if_else(cohort_year!=Inf,1,0))

# estimate the model
pretests <- fixest::feols(
  y ~ evtreat:i(cohort_year, i.year, ref=Inf, ref2 = 2011) | cohort_year + year,
  data = subset(d, treat==0),
  vcov = ~state
)

# joint test of pre-trends
kable(hypotheses(pretests, 
  "b1 + b2 + b3 + b4 + b5 + b6 = 0"), 
  caption = "Joint test that all pre-trends = 0", 
  digits = 3) %>%
  kable_styling()

```

As expected, since we simulated parallel trends, the p-value for this test is quite large (0.90) and there is little evidence to reject the assumption. Now we can also get these estimates to put into a dynamic plot.

```{r etd, message = FALSE, warning = FALSE}
# get the marginal effects
me_e_p <- 
  slopes(pretests, 
    variables = "evtreat", 
    newdata = subset(d, treat==0 & event > -5), 
    by = "event")

# wrangle aggregate ATTs for model summary table
ti_pre <- data.frame(
  term = paste("Pre-treatment(", me_e_p$event, ")", sep=""), 
  estimate = me_e_p$estimate, 
  std.error = me_e_p$std.error,
  conf.low = me_e_p$estimate - 1.96*me_e_p$std.error,
  conf.high = me_e_p$estimate + 1.96*me_e_p$std.error)

etwfe_me_e_p <- list(tidy = ti_pre, glance = gl)
class(etwfe_me_e_p) <- "modelsummary_list"

modelsummary(list("Pre-trends" = etwfe_me_e_p),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
```

Now we can combine these estimates with our earlier dynamic trends for a single plot:

```{r preplot}

ete <- bind_rows(me_e_p, me_e)

ete %>% 
  mutate(`Intervention time` = if_else(event<0, "Pre", "Post")) %>% 
...
As well as comparing these estimates to the 'true' values of the ATTs from our simulated data. 

```{r plot_pre_truth}
ptet <- get_estimates(ete) %>%
  select(term, estimate, conf.low, conf.high) %>%
  mutate(t = c(-4:-1, 0:6)) %>%
  mutate(true_tau = 
    ifelse(t >= 0 & t < 5, ((t + 1) * 1.5), 
...
plot_e_pre <- ptet %>%
  ggplot(aes(x = t, y = estimate)) + 
    ...
    scale_x_continuous(breaks = -4:6) + 
    labs(x = "Relative Time", y = "Estimate") +
    ggtitle("ETWFE event-study regression estimates") +
    scale_color_manual(values = colors) + theme_classic() +
    theme(legend.position = 'bottom',
      legend.title = element_blank(), 
      axis.title = element_text(size = 14),
...
```{r betwfe, warning = FALSE, message = FALSE, cache=T}
# load the brms fitted models
bm1 <- readRDS(here("code/fits", "brms-fe.rds"))
bm2 <- readRDS(here("code/fits", "brms-re-flat.rds"))
bm3 <- readRDS(here("code/fits", "brms-re-inf.rds"))
...
# build the table
modelsummary(list("Fixed only" = bm1, 
 "State RE (non-informative)" = bm2, 
 "State RE (informative)" = bm3), 
...
As can be seen from the table above, the Bayesian implementation produces estimates quite similar to the frequentist ETWFE and CS models. The model with informative priors reduces the standard errors by a small amount, but given the size of the dataset, informative priors are unlikely to have a large impact on either the point estimates or measures of uncertainty, unless they contain a lot of information. 

### Bayesian marginal effects

Getting the marginal effects for different kinds of aggregate ATTs proceeds similarly as above. We use the `marginaleffects` package with the `slopes` function to calculate simple, cohort, calendar, and dynamic effects as we did above. To make this process easier, we refit the model in the format above and included the `treat` variable in creating the saturated product terms. Just to note here also that it made a big difference how these product terms were specified in the `brms` specification. Using a more traditional `x*y*z` method that generates all of the two-way product terms (i.e., `x*y`, `y*z`, etc.) led to significant slowdowns in the model estimation time and coefficients on the 3-way terms between `treat`, `cohort_year` and `year` that were different from the ETWFE models above. However, using the `brms` specification of `x:y:z` just generates the 3-way terms without all of the 2-way terms and gives nearly identical results, as well as being much easier to use for estimating marginal effects. 

The tables below provide marginal estimates for each of the different kinds of aggregated ATTs, and the frequentist estimates are also included for comparison. 

::: panel-tabset

### Simple
```{r b_me_avg, warning = FALSE, message = FALSE, cache=T}
...
# Marginal effect for Bayesian ETWFE (simple)
bme_avg <- slopes(
  bm3, 
...
```{r betwfe_me_c, warning = FALSE, message = FALSE, cache=T}
# Group effects for Bayesian ETWFE (by cohort)
bme_c <- slopes(
  bm3, 
...
```{r betwfe_me_t, warning = FALSE, message = FALSE, cache=T}
# Group effects for Bayesian ETWFE (by cohort)
bme_t <- slopes(
  bm3, 
...
modelsummary(list("ETWFE Time Average" = etwfe_me_t, 
                  "Bayesian Time Average" = betwfe_me_t),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
...
```{r betwfe_me_e, warning = FALSE, message = FALSE, cache=T}
# Need to add event indicators to the dataset 
db <- db %>%
  mutate(event = year - cohort_year)
...
bme_e <- slopes(
  bm3, 
  newdata   = subset(db, treat & event>=0),
  variables = "treat", 
  by        = "event"
  )
...
# wrangle aggregate ATTs for model summary table
bti_e <- data.frame(
  term = paste("ATT(", bme_e$event, ")", sep=""), 
  estimate = bme_e$estimate,
  conf.low = bme_e$conf.low,
  conf.high = bme_e$conf.high,
  std.error = abs(bme_e$conf.high - 
                    bme_e$conf.low) / (2 * 1.96)
)

betwfe_me_e <- list(tidy = bti_e, glance = gl)
class(betwfe_me_e) <- "modelsummary_list"

modelsummary(list("ETWFE Event Study" = etwfe_me_e, 
                  "Bayesian Event Study" = betwfe_me_e),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
...
In general, the marginal estimates from the Bayesian models are effectively the same as from the frequentist models. The standard errors are not uniformly larger or smaller for the Bayesian models. 

## Imputation approach
A couple of recent papers [@Borusyak:2021aa;@Gardner:2022aa] have developed an 'imputation' based approach to DiD with heterogeneous treatment effects. The basic intuition here is to conceptualize the treatment effects as effectively a missing data problem, i.e., what we are trying to do is to impute the counterfactual outcomes for what would have happened to the treated units had they not been treated. 

In the simplest case of DiD without covariates, it turns out that you can do this by running a 'first stage' regression of just unit (or cohort?) and time fixed effects but only for the observations where the treatment has not happened, i.e., $D_{it}=0$. Then, for each observation you effectively subtract out the predicted outcome based only on the fixed effects from the observed outcome values. Then you can construct different averages of those treatment effects. 

For our synthetic dataset, we first run the first stage regression (again, restricted to cohorts and time periods that are prior to treatment), and subtract the observed outcomes from those predicted based on the first stage model. 

```{r fs}
# first stage regression
fs <- fixest::feols(y ~ 0 | state + year, 
  data = subset(d, treat==0), vcov = ~state)
...
We can just look at a simple average of the mean of $Y_{it} - \hat{Y}_{it}(0)$ by treatment status, which gives us the overall ATT. 

```{r imp-mean}

# simple means
dsf %>% group_by(treat) %>%
  summarise(twfe = mean(y_it))
```

Now we can run the regression of our difference measure on different sets of covariates to recover the group-time ATTs, and use similar methods as above to get the marginal ATTs of interest. Below is the comparison to the ETWFE estimates for different kinds of aggregate ATTs.
...
```{r imp_s, message = F}
imp_s <- fixest::feols(
  y_it ~ treat -1, data=dsf, vcov= "hetero")

imp_s_me <- slopes(
  imp_s, 
  newdata   = subset(dsf, treat==1),
  variables = "treat", 
  by        = "treat"
  )

ti <- data.frame(
  term = paste("ATT(", imp_s_me$term, ")", sep = ""),
  estimate = imp_s_me$estimate,
  std.error = imp_s_me$std.error,
  conf.low = imp_s_me$estimate - 1.96*imp_s_me$std.error,
  conf.high = imp_s_me$estimate + 1.96*imp_s_me$std.error)

imp_me_avg <- list(tidy = ti, glance = gl)
class(imp_me_avg) <- "modelsummary_list"

modelsummary(list("ETWFE Simple Average" = etwfe_me_avg, 
  "Imputation Simple Average" = imp_me_avg),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
...
```{r imp_c, message = F}
imp_c <- fixest::feols(
  y_it ~ treat*i(cohort_year, ref(Inf)) -1, 
    data=dsf, vcov= "hetero")

imp_c_me <- slopes(
  imp_c, 
  newdata   = subset(dsf, treat & cohort_year),
  variables = "treat", 
  by        = "cohort_year"
  )

ti <- data.frame(
  term = paste("ATT(g", imp_c_me$cohort_year, ")", sep = ""),
  estimate = imp_c_me$estimate,
  std.error = imp_c_me$std.error,
  conf.low = imp_c_me$estimate - 1.96*imp_c_me$std.error,
  conf.high = imp_c_me$estimate + 1.96*imp_c_me$std.error)

imp_me_c <- list(tidy = ti, glance = gl)
class(imp_me_c) <- "modelsummary_list"

modelsummary(list("ETWFE Cohort Average" = etwfe_me_c, 
  "Imputation Cohort Average" = imp_me_c),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
...
```{r imp_t, message = F}
imp_t <- fixest::feols(
  y_it ~ treat*i(year, ref(2011)) -1, 
    data=dsf, vcov= "hetero")

imp_t_me <- slopes(
  imp_t, 
  newdata   = subset(dsf, treat & year>=2014),
  variables = "treat", 
  by        = "year"
  )

ti <- data.frame(
  term = paste("ATT(", imp_t_me$year, ")", sep = ""),
  estimate = imp_t_me$estimate,
  std.error = imp_t_me$std.error,
  conf.low = imp_t_me$estimate - 1.96*imp_t_me$std.error,
  conf.high = imp_t_me$estimate + 1.96*imp_t_me$std.error)

imp_me_t <- list(tidy = ti, glance = gl)
class(imp_me_t) <- "modelsummary_list"

modelsummary(list("ETWFE Time Average" = etwfe_me_t, 
  "Imputation Time Average" = imp_me_t),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
...
```{r imp_e, message = F}
imp_e <- fixest::feols(
  y_it ~ i(event, ref = c(-Inf, -1)) -1, 
    data=dsf, vcov= "hetero")

imp_e_me <- predictions(
  imp_e, 
  newdata   = subset(dsf, treat & event>=0),
  by        = "event"
  )

ti <- data.frame(
  term = paste("ATT(", imp_e_me$event, ")", sep = ""),
  estimate = imp_e_me$estimate,
  std.error = imp_e_me$std.error,
  conf.low = imp_e_me$estimate - 1.96*imp_e_me$std.error,
  conf.high = imp_e_me$estimate + 1.96*imp_e_me$std.error)

imp_me_e <- list(tidy = ti, glance = gl)
class(imp_me_e) <- "modelsummary_list"

modelsummary(list("ETWFE Time Average" = etwfe_me_e, 
  "Imputation Time Average" = imp_me_e),
  shape = term ~ model + statistic, 
  statistic = c("({std.error})", "conf.int"),
  gof_omit ='._*')
```
