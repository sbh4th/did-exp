...
tt(m_all_table, digits = 2) %>% 
  group_tt(j = list("No cohort FE" = 2:5),
    i = list("Predictions at year 20" = 1, 
      "Slope estimates" = 4))
```

Not right. If the true impacts are an average effect of `r round(te, 2)` by the end of follow-up and an average slope of `r round((1/3) * as_2010$estimate + (2/3) * as_2015$estimate, 2)`, these estimates are not correct. What happens if we just condition on cohort without allowing for separate treatment effects? 
...
Okay, now we get an estimate of `r round(m_all_c_te$estimate, 2)`. Recall that the overall average effect at the end of follow-up was ``r round(te, 2)`. Obviously extrapolation is having some consequence here.
